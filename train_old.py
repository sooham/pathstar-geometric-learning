...
# data
###################################################
# Dataset generation parameters (can be overridden via command line)
graph_d = 250  # Number of spokes/paths in the path-star
graph_l = 5  # Length of each path (number of nodes from root to leaf)
graph_vocab_size = 2000  # Vocabulary size (must be >= d * (l-1) + 1)
assert graph_vocab_size >= graph_d * (graph_l - 1) + 1
graph_holdout_percentage = 0.2  # Percentage of paths to hold out for validation (0.0 to 1.0)
num_pause_tokens = 1  # Number of PAUSE tokens between leaf and path
use_undirected = True  # If True, use undirected edges (both x->y and y->x)
use_directional_tokens = True  # If True, use special tokens (> and <) to indicate edge direction

# Note: Dataset generation and loading happens AFTER configurator.py is executed (see below)
# This allows command-line arguments to override these default values
...
epochs = 50000 # 20000
# Note: TRAIN_DATASET_SIZE, VAL_DATASET_SIZE, batch_per_dataset, eval_iters, max_iters, and wandb_run_name
# will be calculated after the dataset is loaded (see below after configurator.py)
...
warmup_frac = 0.10 # fraction of max_iters to warm up for
lr_decay_frac = 0.99 # fraction of max_iters for warmup + decay phase
...
# After configurator.py is executed, generate/load dataset with potentially overridden parameters
dataset = generate_dataset_if_needed(
    d=graph_d,
    l=graph_l,
    vocab_size=graph_vocab_size,
    holdout_percentage=graph_holdout_percentage,
    num_pause_tokens=num_pause_tokens,
    use_undirected=use_undirected,
    use_directional_tokens=use_directional_tokens
)
...
batch_size = max_allowed_batch_size  # Can be overridden via command line
...
# Calculate learning rate schedule parameters (now that max_iters is defined)
warmup_iters = int(max_iters * warmup_frac)  # how many steps to warm up for
lr_decay_iters = int(max_iters * lr_decay_frac)  # total steps for warmup + decay phase

# Generate wandb run name (now that dataset and batch_size are defined)
